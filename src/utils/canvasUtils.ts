...
//   // ── Step 1: Soft-edge mask (inward-only — no outside bleed) ──────────────
//   //
//   // Original code blurred the mask outward, causing stone to bleed outside
//   // the painted shape. Fix: blur first, then clip back to the original hard
//   // boundary so the feathered edge lives entirely inside the painted area.
//   const featherRadius =
//     cfg.featherRadius === 'auto'
//       ? Math.max(4, Math.min(12, Math.round(w * 0.006)))
//       : cfg.featherRadius;

//   // Hard boundary — exact pixels the user painted
...
//   // Clip blurred mask to hard boundary — feather stays inside, never bleeds out
